-- Visitor Analytics System for Blog Posts
-- This SQL creates tables to track visitor data, IP addresses, locations, and post impressions
...
-- 7. Create function to update visitor analytics summary daily
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(
            (SELECT json_agg(json_build_object('post_slug', post_slug, 'title', title, 'views', view_count))
             FROM (
                 SELECT pi.post_slug, bp.title, COUNT(*) as view_count
                 FROM post_impressions pi
                 LEFT JOIN blog_posts bp ON bp.id = pi.post_id
                 WHERE DATE(pi.timestamp) = summary_date
                 GROUP BY pi.post_slug, bp.title
...
-- 8. Create trigger to auto-update summary (optional - you can also run this via cron)
-- This will update the summary whenever new impressions are added
CREATE OR REPLACE FUNCTION trigger_update_analytics_summary()
RETURNS TRIGGER AS $$
BEGIN
    -- Update summary for today
    PERFORM update_visitor_analytics_summary();
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Add the trigger (optional - might be too frequent for high traffic)
-- CREATE TRIGGER update_analytics_on_impression
--     AFTER INSERT ON post_impressions
--     FOR EACH STATEMENT
--     EXECUTE FUNCTION trigger_update_analytics_summary();

-- 9. Add some sample data for testing (remove in production)
-- INSERT INTO visitor_sessions (session_id, ip_address, country, city, device_type, browser) VALUES
-- ('session_123', '192.168.1.1', 'United States', 'New York', 'desktop', 'Chrome'),
-- ('session_456', '10.0.0.1', 'United Kingdom', 'London', 'mobile', 'Safari'),
-- ('session_789', '172.16.0.1', 'Canada', 'Toronto', 'tablet', 'Firefox');

COMMENT ON TABLE visitor_sessions IS 'Stores visitor session data including IP, location, and device info';
COMMENT ON TABLE post_impressions IS 'Tracks individual post views and engagement metrics';
